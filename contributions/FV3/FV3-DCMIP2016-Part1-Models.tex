...
%\begin{align}
%\pdiff{\rho}{t} &= - \nabla \cdot (\rho \vb{u}), \\
%\pdiff{\vb{u}}{t} &= \left( f\vg{\hat k + \zeta} \right ) \times \vb{u} - \frac{1}{\rho}\nabla p |_z, \\
%\pdiff{\theta_v}{t} &= - \vb{u} \cdot \nabla \theta_v, \\
%\pdiff{w}{t} + \vb{u} \cdot \nabla w &= -\frac{1}{\rho}\pdiff{p}{z} - g,
%\end{align} in terms of Kinetic energy $K = \vb{u} \cdot \vb{u}$, relative vorticity $\vg{\zeta} = \nabla \times \vb{u}$, and local Coriolis parameter $f$. Here, gradient operators (except for that in the pressure-gradient term) are along Lagrangian surfaces.
...
\bibliography{DCMIP2016.bib,FV3.bib}
%%
