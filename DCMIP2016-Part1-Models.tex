...
Model grids are a primary concern for dynamical cores, since they determine how degrees of freedom are distributed across the globe.  In order to meet demands for high computational efficiency and equal partitioning of computation across large parallel systems, modern dynamical cores have been exploring a number of options for model grids.  This section describes many of the model grids that have been employed globally.
...
It is also common to use the 2D Kinetic energy, $K_2 = \frac{1}{2} (\vb{u}_h \cdot \vb{u}_h)$.  Decomposing the momentum transport term into horizontal and vertical components gives
\begin{align}
...
\pdiff{w}{t} =& \vb{u}_h \cdot (\vb{u}_h \cdot \nabla \vb{k}) - \vb{u} \cdot \nabla w - \frac{1}{\rho} \pdiff{p}{z} - \pdiff{\Phi}{z} - (2 \vg{\Omega} \times \vb{u}) \cdot \vb{k}
\end{align}  and
...
The dynamical equations are now formulated in terms of the vertical coordinate $s(t,\vb{x},z)$ with $\partial s / \partial z \neq 0$ everywhere, i.e. following \cite{Kasahara1974} (hereafter K74).  Since $\vb{x}$ and $t$ are shared between the two coordinate systems, the chain rule can be applied to obtain expressions
...
\end{align} which correspond to derivatives in the vertical, in the horizontal and in time.  This final expression is used to describe the rate of change of a quantity on $s$ surfaces.  When applied to $s$, these operators yields the useful identities
\begin{align} \label{eq:NablaZSIdentity}
\nabla_z s = - \left( \pdiff{s}{z} \right) \nabla_s z, \qquad \mbox{and} \qquad \pdiff{s}{t} = - \pdiff{s}{z} \left( \pdiff{z}{t} \right)_s.
...
The prognostic equation for vertical velocity from (\ref{eq:VerticalVelocityOrthogonal}) is then reformulated using (\ref{eq:ChainRule}),
\begin{align} \label{eq:VerticalVelocityOrthogonalS}
\left(\pdiff{w}{t} \right)_s =& \pdiff{s}{z} \left[ \left( \pdiff{z}{t} \right)_s \pdiff{w}{s} - \frac{1}{\rho} \pdiff{p}{s} - \pdiff{}{s} (K + \Phi) \right] - \left[ (\vg{\zeta} + 2 \vg{\Omega}) \times \vb{u}_h \right] \cdot \vb{k}.
\end{align}  Similarly, the prognostic equation for horizontal velocity from (\ref{eq:HorizontalVelocityOrthogonal}) is reformulated as
\begin{align} \label{eq:HorizontalVelocityOrthogonalS}
\left( \pdiff{\vb{u}_h}{t} \right)_s =& - \frac{1}{\rho} \nabla_z p - \nabla_z (K + \Phi) - \left[ (\vg{\zeta} + 2 \vg{\Omega}) \times \vb{u} \right]_z + \left( \pdiff{z}{t} \right)_s (\vb{k} \cdot \nabla \vb{u}_h).
\end{align}  Note that the last term of this expression can then be expanded using
\begin{align}
(\vb{k} \cdot \nabla \vb{u}_h) =& - \vb{u}_h \cdot \nabla \vb{k} - \vb{k} \times (\nabla \times \vb{u}_h),
\end{align} where under the shallow-atmosphere approximation $(\vb{u}_h \cdot \nabla \vb{k})$ is typically dropped.

The prognostic equations utilizing horizontal kinetic energy $K_2$ in place of $K$ are similarly derived from (\ref{eq:PrognosticWEquationK2}),
\begin{align} \label{eq:PrognosticWEquationK2S}
\left( \pdiff{w}{t} \right)_s =& \vb{u}_h \cdot (\vb{u}_h \cdot \nabla \vb{k}) - \vb{u}_h \cdot \nabla w + \left( \pdiff{s}{z} \right) \left\{ \left[ \left( \pdiff{z}{t} \right)_s - w \right] \pdiff{w}{s} - \frac{1}{\rho} \pdiff{p}{s} - \pdiff{}{s} (K_2 + \Phi) \right\} - (2 \vg{\Omega} \times \vb{u}_h) \cdot \vb{k},
\end{align} and from (\ref{eq:PrognosticUhEquationK2}),
\begin{align} \label{eq:PrognosticUhEquationK2S}
\left( \pdiff{\vb{u}_h}{t} \right)_s =& - \frac{1}{\rho} \nabla_z p - \nabla_z (K_2 + \Phi) - \left[ \left( \pdiff{z}{t} \right)_s - w \right] \vb{k} \times (\nabla \times \vb{u}_h)  - \zeta_h \vb{k} \times \vb{u}_h - [2 \vg{\Omega} \times \vb{u}]_z - w (\vb{u}_h \cdot \nabla \vb{k}).
\end{align}  Observe that both of these equations are simplified if $(\partial z / \partial t)_s = w$.

{\color{red}[Is there an easy way to write down the curl (specifically relative vorticity) in arbitrary vertical coordinates?]}

\subsection{Covariant Formulation}

In conjunction with (\ref{eq:HorizontalChainRule}), the horizontal momentum equation (either  (\ref{eq:HorizontalVelocityOrthogonal}) or (\ref{eq:PrognosticUhEquationK2})) with an arbitrary vertical coordinate gives rise to a two-term pressure gradient.  This can be avoided by prognosing the covariant components of the velocity in place of the physical velocity components.  The horizontal partial covariance operator is defined by
\begin{align}
...
\end{align} and so, using (\ref{eq:VerticalVelocityOrthogonalS}) and (\ref{eq:PrognosticUhEquationK2S}) the evolution equation for the covariant horizontal velocity reads
\begin{align}
\left( \pdiff{\vb{v}_h}{t} \right)_s =& \left( \pdiff{\vb{u}_h}{t} \right)_s + \left( \pdiff{w}{t} \right)_s (\nabla_s z) + w \nabla_s \left( \pdiff{z}{t} \right)_s \\
=& - \frac{1}{\rho} \nabla_z p - \nabla_z (K_2 + \Phi) - \left[ \left( \pdiff{z}{t} \right)_s - w \right] \vb{k} \times (\nabla \times \vb{u}_h)  - \zeta_h \vb{k} \times \vb{u}_h - [2 \vg{\Omega} \times \vb{u}]_z - w (\vb{u}_h \cdot \nabla \vb{k}) \\
& + (\nabla_s z) \left\{ \pdiff{s}{z} \left[ \left( \pdiff{z}{t} \right)_s \pdiff{w}{s} - \frac{1}{\rho} \pdiff{p}{s} - \pdiff{}{s} (K_2 + \Phi) - w \pdiff{w}{s} \right] - \left[ (\vg{\zeta} + 2 \vg{\Omega}) \times \vb{u} \right] \cdot \vb{k} \right\} \\
& + w \nabla_s \left( \pdiff{z}{t} \right)_s \\
=& - \frac{1}{\rho} \nabla_s p - \nabla_s (K_2 + \Phi) + w \nabla_s \left( \pdiff{z}{t} \right)_s - w (\vb{u}_h \cdot \nabla \vb{k}) - [( \vg{\zeta} + 2 \vg{\Omega}) \times \vb{u}]_s \nonumber \\
& \qquad - \left[ \left( \pdiff{z}{t} \right)_s - w \right] \left[ \vb{k} \times (\nabla \times \vb{u}_h) - (\nabla_s z) \left( \pdiff{s}{z} \right) \pdiff{w}{s} \right]. \label{eq:CovariantVelocityUpdateS}
\end{align}  Again, observe that this equation simplifies significantly if $(\partial z / \partial t)_s = w$.


%For a time-dependent $s$ coordinate,
%\begin{align}
...
%For a time-dependent $s$ coordinate,
%\begin{align}
%\left[ \pdiff{}{t} (\nabla_s z) \right]_s =& - \left( \nabla_z \pdiff{s}{t} \right) \left( \pdiff{s}{z} \right)^{-1} + (\nabla_z s) \left( \pdiff{s}{z} \right)^{-2} \pdiff{}{z} \pdiff{s}{t} + \left( \pdiff{z}{t} \right)_s (\vb{k} \cdot \nabla) (\nabla_s z) \\
%=& - \left( \nabla_z \pdiff{s}{t} \right) \left( \pdiff{s}{z} \right)^{-1} + (\nabla_z s) \left( \pdiff{s}{z} \right)^{-2} \pdiff{}{z} \pdiff{s}{t} + \left( \pdiff{z}{t} \right)_s (\vb{k} \cdot \nabla) \left[ - (\nabla_z s) \left( \pdiff{s}{z} \right)^{-1} \right] \\
%=& - \left( \pdiff{s}{z} \right)^{-1} \nabla_s \left( \pdiff{s}{t} \right) - \left( \pdiff{z}{t} \right)_s \left( \pdiff{s}{z} \right)^{-1} \nabla_z \left( \pdiff{s}{z} \right) + \left( \pdiff{z}{t} \right)_s \left( \pdiff{s}{z} \right)^{-2} (\nabla_z s) \pdiff{}{z} \pdiff{s}{z} \\
%=& \left( \pdiff{s}{z} \right)^{-1} \left[ - \nabla_s \left( \pdiff{s}{t} \right) - \left( \pdiff{z}{t} \right)_s \nabla_s \left( \pdiff{s}{z} \right)  \right] \\
%=& \nabla_s \left( \pdiff{z}{t} \right)_s
%\end{align}

%In the case of an Eulerian formulation with $(\partial z / \partial t)_s = 0$, we can use
%\begin{align}
%0 = \nabla (\vb{k} \cdot \vb{u}_h) = (\vb{k} \cdot \nabla \vb{u}_h) + (\vb{u}_h \cdot \nabla \vb{k}) + \vb{k} \times (\nabla \times \vb{u}_h)
%\end{align} to obtain
%\begin{align}
%\left( \pdiff{\vb{v}_h}{t} \right)_s =& - \frac{1}{\rho} \nabla_s p - \nabla_s (K + \Phi) - \left[ (\vg{\zeta} + 2 \vg{\Omega}) \times \vb{u} \right]_s - w \left( \pdiff{s}{z} \right)^{-1} \nabla_s \left( \pdiff{s}{t} \right) - \left( \pdiff{z}{t} \right)_s (\vb{k} \cdot \nabla \vb{v}_h) 
%\end{align}

%Although a time-dependent $s$ is admitted by this equation, its fully Eulerian nature makes it difficult to use in this case since degrees of freedom are not guaranteed to align with constant $s$ surfaces.
...
{\color{blue}Define Unstaggered, Lorenz and Charney-Phillips staggering here with figure}
...
This section provide a short description of unique features or design specifications from several modern dynamical cores.  
...
\item A novel Hamiltonian formulation of the equations of motion in non-Eulerian coordinates  \citep{Dubos2014Equations} which is imitated at the discrete level using builing blocks from the litterature \citep{Thuburn2009Numerical,Ringler2010Unified}, and yielding, to a certain extent, an energy-conserving spatial discretization
