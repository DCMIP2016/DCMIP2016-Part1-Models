...
OLAM & NH & $\rho \vb{u}_h$, $\rho w$, $\rho$, $\theta_{il}$, {\color{red} $q$} & Geodesic (\S \ref{sec:grid_geo}) & FV & C-grid \\
...
FV$^3$ & Lorenz & floating mass (\S \ref{sec:FloatingLagrangian}) \\
...
OLAM solves the deep-atmosphere, fully-compressible equations in mass- and momentum-conserving finite-volume form using equations (\ref{eq:ContinuityEquation}), (\ref{eq:PrognosticVelocityFluxForm}), and 
