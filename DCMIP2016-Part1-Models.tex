...
via an application of (\ref{eq:scalar_viscosity_direct}).  Viscosity operations are applied after the completion of the dynamic time step. {\color{red}[Perhaps expand this text a little bit?]}
...
In this section we describe the fluid equations utilized by non-hydrostatic models.  The material derivative is used for  quantities in the Lagrangian frame (following individual air parcels), and is given by
\begin{align}
...
The vorticity-divergence form of the dynamical equations in the height vertical coordinate predicts the (vertical component of) vorticity and (velocity) divergence given by
\begin{align}
\omega_z \equiv \vb{k} \cdot \nabla_s \times \vb{u}_h,
\end{align} and
\begin{align}
D \equiv \nabla_s \cdot \vb{u}_h,
\end{align} respectively, instead of the horizontal velocity.

The vorticity prediction equation is obtained by taking the curl ($\vb{k} \cdot \nabla_s \times$) of the horizontal momentum equation as
\begin{align}
\pdiff{\omega_z}{t} + \nabla_s \cdot (\omega_z^\ast \vb{u}_h) + \pdiff{}{z} (\omega_z^\ast w) - \omega_z \pdiff{w}{z} - \pdiff{}{z} (\vb{k} \times \vb{u}_h) \cdot \nabla_s w + J(c_p \theta_v, \pi) = 0.
\end{align} The Jacobian   is a compact form for the curl of the pressure gradient force, where is the specific heat of air at constant pressure.
...
\subsection{Ullrich-Kinnmark-Gray 5 step 3rd order scheme} \label{sec:UKG53Scheme}
...
{\color{red}[Perhaps move this to paper 2?]}
