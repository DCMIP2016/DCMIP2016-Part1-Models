...
ACME--A & Atmosphere model of the Accelerated Climate Model for Energy & Sandia National Laboratories / University of Colorado, Boulder \\
...
ACME--A & H/NH & $\vb{u}_h$, $w$, $\rho_s$, $\rho_s \theta$, $\Phi$, $\rho_s q$ & Cubed-sphere (\S \ref{sec:grid_cs}) & SE & A-grid \\
ICON & NH (D) & $\vb{u}_h$, $w$, $\rho$, $\theta_v$, $\rho q$ & Icosahedral triangular (\S \ref{sec:grid_ico_tri}) & FV & C-grid \\
...
broad classes of meshes including adaptivity, e.g.~\cite{szmelter2010,kuehnleinJCP2012}.
...
numerical weather prediction with the spectral dynamical core of the IFS at ECMWF in 2016. 
...
\subsubsection{Floating Lagrangian coordinates (DYNAMICO, FV$^3$ and ACME--A)} \label{sec:FloatingLagrangian}
...
\subsection{Prognostic equations and treatment of moisture} \label{sec:PrognosticEquations}
...
governing equations can concisely be written as
...
- \mathbf{k} \frac{g_c}{\theta_{a}} \left(\theta'+\theta_a \left(\frac{q'_v}{\varepsilon} - q'_v - q_c - q_r \right) \right)
...
subscript `a', see \cite{prusa2008eulag,smolarkiewiczJCP2014} for discussion. As far as geometric aspects 
...
subsumes the metric forces in the spherical domain, see \cite{smolarkiewiczetalJCP2017}.
...
the barycentric velocity $\vb{u}_{bc}=\sum_{k} \rho_{k} \vb{u}_{k}/\sum_{k}\rho_{k}$ -- 
...
and in \cite{kurowskiJAS2014,smolarkiewiczetalJCP2017} for extension to moist-precipitating dynamics.  
