...
\end{center}
\caption{An icosahedral (hexagonal) grid.} \label{fig:ICONIcosahedlalHexGrid}
\end{figure}
...
\left(\pdiff{w}{t} \right)_s =& \pdiff{s}{z} \left[ \left( \pdiff{z}{t} \right)_s \pdiff{w}{s} - \frac{1}{\rho} \pdiff{p}{s} - \pdiff{}{s} (K + \Phi) \right] - \left[ (\vg{\zeta} + 2 \vg{\Omega}) \times \vb{u} \right] \cdot \vb{k}.
...
%Then using (\ref{eq:MaterialDerivativeS}) gives
%\begin{align}
