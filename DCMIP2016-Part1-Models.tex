...
\begin{align}
\vb{u} = \vb{u}_h + w \vb{k},
\end{align} where $\vb{k} = \nabla z$ is the unit vector in the vertical direction and $\vb{u}_h \cdot \vb{k} = 0$  ($\vb{u}_h$ is aligned with surfaces of constant $z$).  Note that this decomposition of $\vb{u}$ is not exactly the contravariant form, which instead uses $\nabla s$ in place of $\vb{k}$.  In the orthogonal formulation, the vector-invariant form velocity equation obtained by taking (\ref{eq:PrognosticVelocity2}) $\cdot \vb{k}$ expands as
...
Due to its association with hydrostatic models, it is common to use the 2D Kinetic energy, $K_2 = \frac{1}{2} (\vb{u}_h \cdot \vb{u}_h)$, which admits the relationships
\begin{align}
...
\end{align} where we have used $\nabla \times \vb{k} = 0$.  Thus the vertical velocity equation, obtained by taking (\ref{eq:PrognosticVelocity1}) $\cdot \vb{k}$, leading to
...
\end{align}  Note that under the shallow atmosphere approximation, the metric term $\vb{u}_h \cdot (\vb{u}_h \cdot \nabla \vb{k})$ in (\ref{eq:PrognosticWEquation}) is set equal to zero in accordance with \cite{phillips1966equations}.
...
\dot{s} = \pdiff{s}{t} + \left( \pdiff{s}{z} \right) (w - \vb{v}_h \cdot \nabla_s z).
\end{align}
