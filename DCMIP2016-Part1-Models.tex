...
GEM & NH & $\vb{u}_h$, $w$, {\color{red}$\log \partial s / \partial \zeta$}, $\log T_v$, $q$ & Yin-yang (\S \ref{sec:grid_yinyang}) & FV & C-grid  \\
...
\begin{table}[h]
\caption{Vertical coordinates and staggering for participating dynamical cores.  More details on vertical coordinates can be found in section \ref{sec:VerticalCoordinates}, and details on vertical staggerings can be found in section \ref{sec:VerticalStaggering}.} \label{tab:ModelsVertical}
\begin{center}
\begin{tabular}{cccccc}
\hline Acronym & Vertical Staggering & Vertical Coordinate \\ \hline 
CSU & Lorenz & fixed height \\
DYNAMICO & Lorenz & {\color{red}floating mass?} \\
FV$^3$ & Lorenz & floating mass \\
FVM & Co-located & (floating) height \\
GEM & Modified Charney-Phillips (\S \ref{sec:VerticalStaggering}) & log pressure (\S \ref{sec:GEM_zeta})  \\
HOMME & Co-located & floating mass \\
...
Although the horizontal grid determines the approximate density and distribution of grid points, the placement of prognostic (and diagnostic) variables within each grid volume is still free to be specified.  Hence, in this section we turn our attention to the placement of prognostic variables on the grid and the choice of vertical coordinate.
...
\caption{Horizontal staggering options represented among DCMIP models, in this case depicted on a rectilinear grid and geodesic grid.}
...
The vertical coordinate of GEM model, denoted $\zeta$, is a hybrid terrain-following coordinate of a log-hydrostatic-pressure type.  Taking $s$ (denoted $\pi$ in GEM documentation) as given in (\ref{eq:s_mass-based}), then $\zeta$ is given by the relation
...
- \mathbf{k} \frac{g}{\theta_{a}} \left(\theta'+\theta_a (\frac{q'_v}{\varepsilon} - q'_v - q_c - q_r) \right)
...
subscript "a"; see also \cite{prusa2008eulag,smolarkiewiczJCP2014} for discussion. 
In the shallow- versus deep-atmosphere form of \eqref{fvm:compressible}, gravity is constant 
$g \equiv g_c$ or varies with height as $g = g_c\,(a/(a+z))^2$, respectively.
As far as geometric aspects are concerned, the nabla operator $\overline{\nabla}$ represents
...
subsumes spherical Christoffel symbols \citep[][]{smolarkiewiczetalJCP2017}.
...
The ICON model employs damping and diffusion operators for numerical stabilization and dynamic closure. The details of this scheme appear in sections 2.4 and 2.5 of \cite{zangl2014}, and are summarized here.  For damping, in the corrector step a fourth-order divergence damping term $F_{d}(\mathbf{v})$ is applied in order to allow calling the computationally more expensive diffusion operator (see below) at the physics time steps without incurring numerical stability problems under extreme conditions.
\begin{align}
