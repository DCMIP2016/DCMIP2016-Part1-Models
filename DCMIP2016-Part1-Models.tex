...
{\color{red}[Please assist with fixing notation]}
...
\begin{align}
\mu = \pdiff{p}{\pi_h} - 1,
\end{align} in which case the equations of GEM model \citep{Girard2014} are
\begin{align}
\frac{d \vb{u}_h }{dt} + f {\bf k} \times {\bf{v}_{h}} + R_d T_v\nabla_\zeta \,  \log\, p+ \left( 1 + \mu \right) \nabla_\zeta {\Phi} =& 0, \\
\frac{d w}{dt}- g \mu =& 0, \\
...
A vertical sponge layer, which also uses the Laplacian, is employed on wind components and $T_v$ with a vertical modulation on the topmost levels.  For stabilization purpose, the temporal discretization of GEM also uses an off-centering parameter.  Shape-preserving solutions to the tracer advection equation are also computed for tracers by using the quasi-monotone semi-Lagrangian (QMSL) method \citep{Bermejo1992QMSL}. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Temporal Discretizations} \label{sec:TemporalDiscretizations}

A wide variety of temporal discretizations available among global atmospheric models, particularly to address issues related to the large horizontal-vertical aspect ratio typical in these systems.  In this section we briefly discuss several modern approaches that are presently employed.
...
{\color{red}[Introductory sentence that states the difference between this scheme and the approaches above.]}   Consider a frictionless adiabatic prognostic equation of the form:
\begin{equation} 
\frac {dF}{dt}+G=0,
