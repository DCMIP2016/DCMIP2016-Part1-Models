...
%DIF DEL DCMIP2016-Part1-Models-20170604.tex   Thu Aug 10 12:06:48 2017
%DIF ADD DCMIP2016-Part1-Models.tex            Thu Aug 10 12:05:13 2017
%% ---------------------------------
...
%DIF 15-20d15
%DIF <  \usepackage{xcolor} \usepackage{ulem} \usepackage{changebar}
...
Vertical operators are based on the mimetic (mass and energy conserving) 2nd order finite difference discretization of \cite{simmons1981energy}. All fields are co-located in the horizontal, in the sense that they share the same 4th-order basis functions.  \DIFaddbegin \DIFadd{By default, tracer transport is sub-cycled relative to the hydrodynamics, but also performed using the spectral element method using the tracer mass as a prognostic variable.
}\DIFaddend 
...
The CSU model \DIFdelbegin \DIFdel{uses }\DIFdelend \DIFaddbegin \DIFadd{is a finite-volume model using }\DIFaddend an optimized geodesic grid \DIFdelbegin \DIFdel{to discretize the sphere }\DIFdelend \citep{heikes1995numerical,heikes2013optimized}, with height as the vertical coordinate. The model is based on the non-hydrostatic Unified System of equations proposed by \cite{arakawa2009unification}, which filters vertically propagating sound waves but allows the Lamb wave and does not require a reference state. The horizontal wind field is determined by predicting the vertical component of the vorticity and the divergence of the horizontal wind, and then solving a pair of two-dimensional Poisson equations for a stream function and velocity potential. Horizontal diffusion is included in the form of a fourth-order \DIFdelbegin \DIFdel{Laplacian }\DIFdelend \DIFaddbegin \DIFadd{hyperviscosity }\DIFaddend operator applied on constant height surfaces ($\nabla^4_z$) that acts on the vorticity, divergence, potential temperature, and tracer.  The CSU model supports both third-order and fifth-order upstream-weighted finite-volume advection schemes, with positivity preservation enforced via mass borrowing.
\subsection{DYNAMICO}

DYNAMICO is a mimetic finite-difference / finite-volume model \DIFdelbegin \DIFdel{, solving initially the hydrostatic primitive equations and }\DIFdelend \DIFaddbegin \DIFadd{using a geodesic grid and a floating vertical mass coordinate.  Although originally a hydrostatic model, it has been }\DIFaddend recently extended to solve the shallow-atmosphere \DIFdelbegin \DIFdel{fully-compressible }\DIFdelend \DIFaddbegin \DIFadd{non-hydrostatic Euler }\DIFaddend equations. DYNAMICO's design uniquely combines a representation of the prognostic and diagnostic fields following the ideas of discrete differential geometry \DIFdelbegin \DIFdel{, }\DIFdelend \citep{Dubos2015DYNAMICO10}.  It includes a novel Hamiltonian formulation of the equations of motion in non-Eulerian coordinates \citep{Dubos2014Equations} which is imitated at the discrete level using building blocks from the literature \citep{thuburn2009numerical,ringler2010unified}, and (up to the addition of explicit diffusion) leads to an energy-conserving spatial discretization.  It also incorporates a novel explicit-implicit splitting which results in a simple, efficient and scalable implicit solver while allowing stable time steps close or identical to those of the hydrostatic solver (Dubos and Dubey, in preparation).  \DIFaddbegin \DIFadd{Horizontal diffusion is included via a fourth-order hyperviscosity operator.  }\DIFaddend In addition, it features a conservative positive-definite transport scheme based on a slope-limited finite-volume approach \citep{Dubey2015Intercomparison}.

\subsection{FV Cubed (FV$^3$)}

The GFDL Finite-Volume Cubed-Sphere Dynamical Core (FV$^3$, or sometimes written FV3) is a \DIFdelbegin \DIFdel{fully }\DIFdelend finite-volume \DIFdelbegin \DIFdel{discretization of the fully-compressible }\DIFdelend \DIFaddbegin \DIFadd{model that solves the }\DIFaddend non-hydrostatic Euler equations on the equiangular gnomonic cubed-sphere grid with \DIFdelbegin \DIFdel{flow-following }\DIFdelend \DIFaddbegin \DIFadd{a floating }\DIFaddend Lagrangian vertical coordinate\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{lin2004vertically}}%DIFAUXCMD
}\DIFdelend . The Lagrangian vertical coordinate deforms so that the flow is constrained to follow the Lagrangian surfaces, allowing vertical transport to be represented implicitly without additional advection terms (see section \ref{sec:FloatingLagrangian} below). \DIFaddbegin \DIFadd{The non-hydrostatic formulation extends the hydrostatic model described in  \mbox{%DIFAUXCMD
\cite{lin2004vertically} }%DIFAUXCMD
by adding a prognostic vertical velocity and geometric height of each grid cell, which can then be used to compute density.  The discretization is on the C-D grid as described by \mbox{%DIFAUXCMD
\citet{LR1997QJR} }%DIFAUXCMD
(also see section \ref{sec:HorizontalStaggering}), although the prognostic horizontal winds are stored in the native Gnomonic local coordinate.  All variables are 3D cell-mean values, except for the horizontal winds, which are 2D face-mean values on their respective staggerings; as a result, diagnostic vorticity is a 3D cell-mean value.  }\DIFaddend Fluxes are computed using the Piecewise-Parabolic Method of \citet{CW1984JCP} with an optional monotonicity constraint; in non-hydrostatic applications the monotonicity constraint is used primarily for tracer transport. \DIFdelbegin \DIFdel{The discretization is on the C-D grid as described by \mbox{%DIFAUXCMD
\citet{LR1997QJR} }%DIFAUXCMD
(also see section \ref{sec:HorizontalStaggering}). }\DIFdelend Since divergence is effectively invisible to the solver, a divergence damping is applied to control numerical noise as divergent modes cascade to the grid scale.  Implicit viscosity is applied through the monotonicity constraint; if non-monotonic advection is used for the momentum and total air mass a weak explicit hyperviscosity is applied for stability and to alleviate numerical noise. Explicit viscosity is applied every acoustic timestep. 

\DIFdelbegin \DIFdel{The prognostic horizontal winds are stored in the native Gnomonic local coordinate. The non-hydrostatic solver adds a prognostic vertical velocity and geometric height of each grid cell, which can then be used to compute density. All variables are 3D cell-mean values, except for the horizontal winds, which are 2D face-mean values on their respective staggerings; as a result, diagnostic vorticity is a 3D cell-mean value.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Finite-Volume Module (FVM) of the Integrated Forecasting System}
\label{sec:FVM}

The Finite-Volume Module (FVM) of the Integrated Forecasting System (IFS) is currently under development at 
ECMWF \citep{smolarkiewiczetalJCP2016,kuehnleinJCP2017,smolarkiewiczetalJCP2017}. FVM \DIFdelbegin \DIFdel{solves the 
compressible Euler equations in a geospherical framework \mbox{%DIFAUXCMD
\citep{szmelter2010,smolarkiewiczetalJCP2016}}%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin \DIFadd{is a fully-unstructured finite volume model that solves the 
non-hydrostatic Euler equations on an octahedral reduced Gaussian grid with fixed height vertical coordinate \mbox{%DIFAUXCMD
\citep{szmelter2010,smolarkiewiczetalJCP2016}}%DIFAUXCMD
. 
The horizontal spatial discretization uses the median-dual approach, combined with a structured finite-difference discretization in the vertical. In both the horizontal and 
the vertical discretization all variables are co-located. }\DIFaddend A centered two-time-level semi-implicit 
...
advection scheme \citep{smolarkiewiczszmelter2005,kuehnleinJCP2017}.  The \DIFdelbegin \DIFdel{horizontal spatial discretization is fully unstructured finite-volume using the median-dual approach. This is 
combined with a structured-grid finite-difference discretization in the vertical direction.  
In both the horizontal and 
the vertical discretization all variables are co-located. 
The median-dual finite-volume mesh in the horizontal is developed about the nodes of the }\DIFdelend \DIFaddbegin \DIFadd{monotonicity constraint in MPDATA also provides sufficient diffusion to stabilize the model, so no other explicit diffusion mechanism is required.  
Note that the }\DIFaddend octahedral reduced Gaussian 
grid \DIFdelbegin \DIFdel{(Section~\ref{subsection:octahedralreducedgaussiangrid}). The octahedral reduced Gaussian 
grid }\DIFdelend is also employed in the spectral-transform dynamical core of the presently operational IFS at ECMWF, which facilitates 
...
%DIF < No explicit diffusion or filtering is applied in FVM for DCMIP, unless it is required for particular test cases. 

%DIF < The finite-volume module (FVM) of the Integrated Forecasting System (IFS) is presently under development at 
%DIF < ECMWF \citep{smolarkiewiczetalJCP2016}. FVM solves the fully compressible Euler 
%DIF < equations in geospherical coordinates. Both deep-atmosphere and shallow-atmosphere equations 
%DIF < are available by means of simple switches. The formulation incorporates 
%DIF < a generalized, optionally time-dependent, terrain-following vertical coordinate based on height. 
%DIF < A centered two-time-level semi-implicit integration scheme is employed with 3D implicit treatment 
%DIF < of acoustic, buoyant, and rotational modes \citep{smolarkiewiczJCP2014} (see section \ref{sec:FVMSemiImplicit}). The associated 3D 
%DIF < Helmholtz problem is solved iteratively using a bespoke preconditioned Generalised 
%DIF < Conjugate Residual approach. The dynamics time step is adapted 
%DIF < at every time step according to a given maximum advective CFL number (typically somewhat smaller 
%DIF < than 1). The physics time step is identical to the dynamics time step. 
%DIF <  
%DIF < The integration procedure uses the multidimensional flux-form Eulerian non-oscillatory MPDATA 
%DIF < advection scheme \citep{smolarkiewiczszmelter2005,kuehnleinJCP2017}. 
%DIF < The horizontal spatial discretization is fully unstructured finite-volume using the 
%DIF < median-dual approach. This is combined with a structured-grid finite-difference approach in the 
%DIF < vertical direction; see \cite{smolarkiewiczetalJCP2016} for an exposition. 
%DIF < In both the horizontal and the vertical discretization, all prognostic variables are 
%DIF < co-located. The median-dual finite-volume mesh in the horizontal is developed about the points/nodes 
%DIF < of the octahedral reduced Gaussian grid (see section \ref{sec:grid_oct}). The octahedral 
%DIF < reduced Gaussian grid is also employed in the spectral dynamical core of the current operational 
%DIF < IFS at ECMWF, which facilitates interoperability of the two formulations. However, we note that FVM is 
%DIF < not restricted to this grid and offers capabilities towards a broad classes of meshes including adaptivity. 
%DIF < 
%DIF < No explicit diffusion is applied in FVM for DCMIP, apart from the momentum dissipation and scalar 
%DIF < diffusion required for some of the test cases, which is the vertical dissipation/diffusion in the planetary
%DIF < boundary layer parametrization and the constant-coefficient second-order dissipation/diffusion in 
%DIF < the supercell test. An absorbing layer in the first latitude ring around the poles is optionally used in 
%DIF < the form of a Rayleigh-type forcing to the prognostic variables. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < The first row of the table below refers to the size of the octahedral reduced Gaussian grid, where 
%DIF < the number following "O" (for octahedral) denotes the number of latitudes between pole and equator.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Neither explicit diffusion/filters nor decentering of the time-stepping scheme is applied in DCMIP, 
%DIF < No explicit diffusion is applied in FVM in DCMIP, apart from the explicit momentum dissipation and 
%DIF < scalar diffusion required for some of the test cases.  FVM uses adaptive time stepping for optimal
%DIF < efficiency and accuracy. 
 %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < FVM employs distributed- and shared-memory parallelisation using the MPI and OpenMP standards, 
%DIF < respectively. %The underlying distributed-memory data structures in FVM are provided by the Atlas 
%DIF < framework developed at ECMWF. 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Global Environmental Multiscale (GEM) model} \label{sec:GEM_core}

The GEM model \citep{Girard2014} is used for operational forecasting at Environment and Climate Change Canada.  \DIFdelbegin \DIFdel{The horizontal discretization uses the }\DIFdelend \DIFaddbegin \DIFadd{GEM solves the non-hydrostatic Euler equations on the }\DIFaddend Yin-Yang grid \citep{kageama2004yinyang} with Arakawa C-grid staggering \DIFaddbegin \DIFadd{of prognostic variables}\DIFaddend .
...
For this problem, a direct solver is involved, using the Schwarz-type domain decomposition method \citep{Qaddouri2008schwarz}.  \DIFdelbegin \DIFdel{The dynamics and physics are time split}\DIFdelend \DIFaddbegin \DIFadd{Semi-Lagrangian advection is also used for tracer transport}\DIFaddend .
To eliminate numerical noise, an explicit hyperviscosity is employed for wind components and tracers
via applications of the Laplacian operator, applied after the completion of the physics time step.

\subsection{\DIFdelbegin \DIFdel{Icosahedral }\DIFdelend \DIFaddbegin \DIFadd{ICOsahedral }\DIFaddend Non-hydrostatic model (ICON)}

The ICON model \citep{zangl2015icon} \DIFdelbegin \DIFdel{discretizes the compressible equations for a shallow atmosphere in }\DIFdelend \DIFaddbegin \DIFadd{is a finite-volume model that solves the non-hydrostatic Euler equations in 2D }\DIFaddend vector invariant form \DIFdelbegin \DIFdel{for the horizontal wind on a triangular}\DIFdelend \DIFaddbegin \DIFadd{on an icosahedral (triangular) }\DIFaddend Arakawa C-grid\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, utilizing }\DIFaddend a smoothed terrain-following height-based Lorenz \DIFdelbegin \DIFdel{grid}\DIFdelend \DIFaddbegin \DIFadd{vertical discretization}\DIFaddend .  Prognostic horizontal velocities are stored as normal wind components \DIFdelbegin \DIFdel{$v_n$ }\DIFdelend at the edge mid-points of full levels.  Prognostic vertical \DIFdelbegin \DIFdel{wind $w$ }\DIFdelend \DIFaddbegin \DIFadd{velocity }\DIFaddend is stored at the circumcenters of the triangles on half levels. The discretization employs a two \DIFdelbegin \DIFdel{time level }\DIFdelend \DIFaddbegin \DIFadd{time-level }\DIFaddend predictor corrector scheme, which is explicit in all terms except for those describing the vertical propagation of sound waves.  \DIFdelbegin \DIFdel{Time splitting is applied between the dynamics that is forced by slow physics on the one hand and horizontal diffusion, tracer transport, and fast physics. One complete time step typically includes 5 dynamical sub-steps. The average air mass flux of the dynamical  sub-steps is provided to the tracer transport to allow for a mass-consistent transport. }\DIFdelend For stabilization of the divergence term on the triangular C-grid the divergence in a triangle is computed from modified normal wind components resulting from a weighted average including normal winds on edges of adjacent cells. Further divergence damping is applied to the normal wind at every sub-step. Rayleigh damping is applied to the vertical wind in layers close to the model top in order to avoid the reflection of gravity waves. The horizontal diffusion, which is applied at full model time steps, combines a \DIFdelbegin \DIFdel{flow dependent }\DIFdelend \DIFaddbegin \DIFadd{flow-dependent }\DIFaddend Smagorinsky scheme with a background 4th-order Laplacian diffusion operator. For tracer transport a flux form semi-Lagrangian scheme with monotone flux limiters is used, which \DIFdelbegin \DIFdel{grants }\DIFdelend \DIFaddbegin \DIFadd{leads to }\DIFaddend local mass conservation and consistency with the air motion. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Specifically, the average air mass flux of the dynamical  sub-steps is provided to the tracer transport to allow for a mass-consistent transport. These }\DIFaddend numerical methods have been chosen for high numerical efficiency, and they rely on next-neighbor communication only, thus allowing massive parallelization.

%DIF > Time splitting is applied between the dynamics that is forced by slow physics on the one hand and horizontal diffusion, tracer transport, and fast physics. One complete time step typically includes 5 dynamical sub-steps.
\DIFaddbegin 

\DIFaddend \subsection{Model for Prediction Across Scales (MPAS)}

The Model for Prediction Across Scales (MPAS) \citep{skamarock2012multiscale} \DIFdelbegin \DIFdel{uses }\DIFdelend \DIFaddbegin \DIFadd{is a finite-volume model that solves the non-hydrostatic Euler equations using }\DIFaddend an Arakawa C-grid \DIFdelbegin \DIFdel{built }\DIFdelend on a centroidal Voronoi tessellation \DIFaddbegin \DIFadd{grid }\DIFaddend (see section \ref{sec:grid_ccvt}), \DIFdelbegin \DIFdel{in conjunction with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the mimetic TRiSK discretization \citep{thuburn2009numerical,ringler2010unified}.  \DIFdelbegin \DIFdel{Advection terms are nominally third- to fourth-order and are handled in accordance with \mbox{%DIFAUXCMD
\cite{skamarock2011conservative}}%DIFAUXCMD
.  }\DIFdelend In the vertical, MPAS employs a Lorenz-type second-order nodal finite volume method with a smoothed terrain-following height coordinate.  \DIFaddbegin \DIFadd{Advection is nominally third- to fourth-order and are handled in accordance with \mbox{%DIFAUXCMD
\cite{skamarock2011conservative}}%DIFAUXCMD
.  }\DIFaddend The prognostic variables are dry air pseudodensity $\tilde{\rho}_d$, dry momentum $\tilde{\rho}_d \vb{u}$, and a modified moist potential temperature.  Integration in time is handled via the split-explicit method of \cite{klemp2007conservative}.  Various filters are available for controlling spurious oscillations, including Smagorinsky-type eddy viscosity, and fourth-order hyperdiffusion.

\subsection{Non-hydrostatic ICosahedral Atmospheric Model (NICAM)}

NICAM \DIFdelbegin \DIFdel{uses the }\DIFdelend \DIFaddbegin \DIFadd{is a }\DIFaddend finite-volume \DIFdelbegin \DIFdel{discretization of the fully compressible }\DIFdelend \DIFaddbegin \DIFadd{model that solves the }\DIFaddend non-hydrostatic Euler equations \DIFdelbegin \DIFdel{on a }\DIFdelend \DIFaddbegin \DIFadd{using an optimized }\DIFaddend geodesic grid optimized by the spring dynamics \DIFaddbegin \DIFadd{using the }\DIFaddend method proposed by \cite{tomita2002optimization}. \DIFdelbegin \DIFdel{The system of equations is transformed to the standard }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend terrain-following height coordinate system \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{tomita2004new}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{is used in the vertical \mbox{%DIFAUXCMD
\citep{tomita2004new} }%DIFAUXCMD
with Lorenz staggering}\DIFaddend . Instead of \DIFdelbegin \DIFdel{the temperature or the }\DIFdelend \DIFaddbegin \DIFadd{temperature or }\DIFaddend potential temperature, \DIFdelbegin \DIFdel{the }\DIFdelend total energy is \DIFdelbegin \DIFdel{chosen as a prognostic variable, }\DIFdelend \DIFaddbegin \DIFadd{prognosed }\DIFaddend following the method of \citep{satoh2002conservative, satoh2003conservative}. All \DIFdelbegin \DIFdel{of the }\DIFdelend prognostic variables are collocated horizontally at the mass centroid of each hexagonal/pentagonal cell to mitigate accuracy reduction under cell averaging, which is required in converting \DIFdelbegin \DIFdel{cell integrated }\DIFdelend \DIFaddbegin \DIFadd{cell-integrated }\DIFaddend quantities to point values at cell centroids. The use of cell centroids ensures quasi second-order accuracy of the gradient and divergence operators of NICAM \citep{tomita2001shallow}. \DIFdelbegin \DIFdel{The Arakawa C-grid staggering is used vertically. A }\DIFdelend \DIFaddbegin \DIFadd{For integration in time, a }\DIFaddend two-stage Runge-Kutta scheme is usually \DIFdelbegin \DIFdel{used to reduce computational burden }\DIFdelend \DIFaddbegin \DIFadd{employed because of low computational cost, }\DIFaddend although a three-stage Runge-Kutta scheme \citep{wicker2002time} is \DIFaddbegin \DIFadd{available and }\DIFaddend recommended. The split-explicit time discretization is used for the horizontally propagating sound waves with the divergence damping term \citep{skamarock1992stability}. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend implicit time discretization is adopted for the vertically propagating \DIFdelbegin \DIFdel{one}\DIFdelend \DIFaddbegin \DIFadd{wave modes}\DIFaddend . A variant of the piecewise linear transport scheme \citep{miura2007upwind, niwa2011three} is used with a flux limiter of \cite{thuburn1997pv} for passive tracer transports.

\subsection{Ocean-Land-Atmosphere Model (OLAM)}

OLAM \citep{walko2008ocean, walko2008ocean2, walko2011direct} is a \DIFdelbegin \DIFdel{deep-atmosphere, fully-compressible model }\DIFdelend \DIFaddbegin \DIFadd{finite-volume mode }\DIFaddend that solves the \DIFdelbegin \DIFdel{equations of motion in finite-volume }\DIFdelend \DIFaddbegin \DIFadd{deep-atmosphere non-hydrostatic Euler equations in }\DIFaddend momentum conservation form \DIFdelbegin \DIFdel{. Acoustic modes are solved explicitly in the horizontal and implicitly in the vertical by means of time splitting. Equations are discretized with a C-grid formulation }\DIFdelend on a hexagonal Voronoi \DIFdelbegin \DIFdel{mesh with }\DIFdelend \DIFaddbegin \DIFadd{grid with Arakawa C-grid staggering.  The model supports }\DIFaddend optional local mesh refinement\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend which introduces some pentagons and heptagons \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{to the grid}\DIFaddend .  Height is the vertical coordinate, and a Lorenz vertical grid staggering is used.  A unique feature of OLAM is that grid levels are horizontal and intersect topography. This avoids a number of well-documented errors associated with terrain-following grids and also eliminates the need for evaluation of coordinate transformation terms. Topography is represented as a smooth (non-stepped) surface by means of cut cells whose surfaces and volume are reduced according to the portion of each cell that is below ground. The OLAM cut-cell formulation conserves mass and momentum.  \DIFaddbegin \DIFadd{Acoustic modes are solved explicitly in the horizontal using a 2nd-order Adams-Bashforth method and implicitly in the vertical by means of time splitting.  Tracer transport is 2nd-order in space and time, using the scheme of \mbox{%DIFAUXCMD
\cite{crowley1968numerical} }%DIFAUXCMD
for obtaining consistent numerical fluxes, and Lax-Wendroff for time integration.
}\DIFaddend 

%DIF > , fully-compressible model that solves the equations of motion in finite-volume momentum conservation form.  Equations are discretized with a C-grid formulation on a hexagonal Voronoi mesh with optional local mesh refinement (which introduces some pentagons and heptagons). 
\DIFaddbegin 

\DIFaddend \subsection{TEMPEST}

%DIF > Based upon the sections for the rest of the paper I would like to be
%DIF > able to ascertain the following properties for each model from this section: equation
%DIF > set, horizontal grid and discretization, vertical grid and discretization, temporal
%DIF > discretization, principal diffusion and stabilization mechanisms and transport
%DIF > scheme.
\DIFaddbegin 

%DIF > The GFDL Finite-Volume Cubed-Sphere Dynamical Core (FV$^3$, or sometimes written FV3) is a finite-volume model that solves the fully-compressible non-hydrostatic Euler equations on the equiangular gnomonic cubed-sphere grid with a floating Lagrangian vertical coordinate.

\DIFaddend The Tempest model \citep{ullrich2014global, guerra2016high} is an experimental testbed for high-performance numerical methods that \DIFdelbegin \DIFdel{uses a horizontal }\DIFdelend \DIFaddbegin \DIFadd{solves the non-hydrostatic Euler equations on a cubed-sphere grid using a horizontally co-located }\DIFaddend spectral element discretization\DIFdelbegin \DIFdel{and vertical nodal finite volume method }\DIFdelend \DIFaddbegin \DIFadd{.  In the vertical Tempest uses an Eulerian finite-volume discretization }\DIFaddend with Lorenz staggering \DIFdelbegin \DIFdel{based on the cubed-sphere grid with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend terrain-following \DIFdelbegin \DIFdel{height-based }\DIFdelend \DIFaddbegin \DIFadd{height }\DIFaddend coordinate.  The implementation includes both fully explicit time integration, and a horizontally-explicit vertically-implicit formulation that is solved with a third-order implicit-explicit additive Runge-Kutta scheme from \cite{ascher1997implicit}.  \DIFaddbegin \DIFadd{Fourth-order hyperviscosity is used in the horizontal to prevent a buildup of energy at the grid scale.  The model further provides an optional upwind-biased transport scheme in the vertical column.  Tracer transport is performed using the spectral element method with the same timestep as the hydrodynamics and using the tracer mass density as a prognostic variable.  As with the hydrodynamics, tracer transport is performed explicitly in the horizontal and implicitly in the vertical.
}\DIFaddend 
...
The classic latitude-longitude grid \DIFdelbegin \DIFdel{consists of a subdivision of the sphere }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend produced by subdividing \DIFaddbegin \DIFadd{the sphere }\DIFaddend along lines of constant latitude and longitude.  \DIFaddbegin \DIFadd{The latitude-longitude grid has the benefits of being globally rectilinear, which simplifies data access and subdivision of computation across processors, and yields a vector basis that is locally orthogonal nearly everywhere.  This structure accurately maintains purely zonal flows and simplifies data post-processing for visualization.  }\DIFaddend Because of the convergence of grid lines near the poles, the operational use of this grid requires that the associated numerical scheme be resilient to arbitrarily small Courant number, or that polar filtering be employed to remove unstable computational modes \citep{lin2004vertically}.  This grid is employed by the UK Met Office \citep{davies2005new, wood2014inherently}.
...
Given a set of $N$ distinct points on the sphere $x_i$ (referred to as the generators, $1 \leq i \leq N$), the \textit{Voronoi tessellation} (or the \textit{Voronoi diagram}) associated with the generators is the set of polygons $\Omega_i$ consisting of all points that are closer (in the sense of great-circle distance) to $x_i$ than any other $x_j$ with $i \neq j$ \citep{okabe2009spatial}.  For a given set of generators, this tiling is unique and completely covers the sphere, and so can be employed in conjunction with many finite volume methods.  However, for an arbitrary set of generators it is easy to produce highly distorted polygons, particularly if the density of generators varies substantially.  This has led to the development of \textit{constrained centroidal Voronoi tessellation (CCVT)} \citep{du2003constrained}, which imposes the additional requirement that the set of generators be coincident with the centroids of each polygon.  Given a desired polygonal density function, several algorithms have been developed to generate CCVTs both in Cartesian and spherical geometry (i.e. for ocean basins or ice sheets) \citep{ringler2008multiresolution}.  Figure \ref{fig:CCVT} depicts one such CCVT grid that is compatible with the MPAS model.  CCVT grids are often confused with deformations of the icosahedral (hexagonal) grid described in section \ref{sec:grid_geo}, since both typically contain a large number of hexagonal elements.\DIFdelbegin \DIFdel{However}\DIFdelend \DIFaddbegin \DIFadd{; however}\DIFaddend , CCVT grids are fundamentally constructed using a very different technique\DIFdelbegin \DIFdel{and will often contain polygonal elements }\DIFdelend \DIFaddbegin \DIFadd{.  Although hexagons are, by far, the most common polygon on CCVT grids, CCVT grids on the sphere will also include at least 12 pentagons and sometimes other polygons }\DIFaddend with more than six sides.  \DIFaddbegin \DIFadd{Quadrilateral elements are theoretically possible, but are never found in practice on the final grid due to being a locally unstable solution of the underlying CCVT system of equations.
}\DIFaddend 
...
The horizontal placement of variables impacts a number of properties of the numerical method, including how energy and enstrophy conservation is managed, any computational modes that might arise due to differencing, dispersion properties, and the maximum stable timestep size \DIFaddbegin \DIFadd{for explicit timestepping schemes }\DIFaddend \citep{randall1994geostrophic, ullrich2014understanding}.  The original four Arakawa grids \citep{arakawa1977computational}, denoted with letters A- through D- were initially designed for rectilinear meshes, but were later adapted for a variety of unstructured grids.  Later, other grid types were added, including the Z-grid, which used the vertical component of vorticity and the horizontal divergence in place of the velocity components \citep{randall1994geostrophic}, and the ZM-grid, which extends the B-grid to hexagons by placing the velocity at hexagonal nodes \citep{ringler2002zm}.  By interpreting ``staggerings'' to be analogous to a choice of finite element basis, new staggerings are under development in the context of mixed finite element methods \citep{cotter2012mixed}.  Among the models that participated in DCMIP, only four grids were represented:  The A-grid, which involves simple co-location of all velocity components \DIFaddbegin \DIFadd{and scalar fields}\DIFaddend ; the C-grid, which places perpendicular velocity components on grid edges; the D-grid, which places parallel velocity components on grid edges; and the Z-grid\DIFaddbegin \DIFadd{, which co-locates the vorticity, divergence and buoyancy variables }\DIFaddend (see Figure \ref{fig:horizontalstaggering}).

Arguments in favor or against particular staggerings have generally emerged from linear analyses, and typically in the absence of either implicit or explicit diffusion.  In this context, the A-grid tends to support large time step sizes, but produces unphysical phase speeds and negative group velocities at high wavenumbers, including a stationary $2 \Delta x$ wavelength mode (even in the context of finite element methods); the C-grid better represents short wave modes, does not support extraneous computational modes (as long as the number of \DIFdelbegin \DIFdel{edges }\DIFdelend \DIFaddbegin \DIFadd{horizontal faces }\DIFaddend is equal to twice the number of volumes), but \DIFdelbegin \DIFdel{also requires a timestep that is half that of }\DIFdelend \DIFaddbegin \DIFadd{typically has a more restrictive timestep with explicit timestepping schemes than }\DIFaddend the A-grid\DIFdelbegin \DIFdel{(in the 2nd order case), or 40\% smaller (in the 4th order case) \mbox{%DIFAUXCMD
\citep{ullrich2014understanding}}%DIFAUXCMD
}\DIFdelend ; the D-grid provides a better representation of vorticity, but produces unphysical effects analogous to those on the A-grid at high wavenumbers that must be controlled with divergence damping; finally, the Z-grid yields optimal dispersion properties, but requires the inversion of a \DIFdelbegin \DIFdel{Helmholtz }\DIFdelend \DIFaddbegin \DIFadd{Poisson }\DIFaddend problem at each timestep to extract the velocity field from the divergence and vorticity.
...
\DIFaddbegin \subsection{\DIFadd{CSU}} \label{sec:CSUEquations}

\DIFadd{The CSU model uses the vorticity-divergence form of the equations of motion, as described in section \ref{sec:VorticityDivergenceForm}, discretized on the geodesic mesh with absolute vorticity and velocity divergence scalars stored at cell-centers.  The unified approximation of the equations of motion \mbox{%DIFAUXCMD
\citep{arakawa2009unification} }%DIFAUXCMD
...
\DIFaddbegin \subsection{\DIFadd{MPAS}} \label{sec:MPASEquations}

\DIFadd{The evolution equations used by MPAS are fully described in \mbox{%DIFAUXCMD
\cite{skamarock2012multiscale}}%DIFAUXCMD
, based on the formulation of \mbox{%DIFAUXCMD
\cite{dutton1986ceaseless}}%DIFAUXCMD
.  The MPAS model uses the momentum form of the update equations, as described in section \ref{sec:MomentumForm}, with dry mass utilized for the density variable $\tilde{\rho}_s$.  MPAS further evolves dry mass using a continuity equation of the form (\ref{eq:EulerianContinuityEquation}) and moist potential temperature following (\ref{eq:PotentialTemperatureFluxForm}).
}

\subsection{\DIFadd{NICAM}} \label{sec:NICAMEquations}

\DIFadd{NICAM prognoses horizontal and vertical momentum analogous to the approach described in section \ref{sec:MomentumForm}.  It further evolves the density perturbation from the background reference state using (\ref{eq:EulerianContinuityEquation}) and sensible heat part of internal energy.  A detailed explanation of the evolution equations can be found in \mbox{%DIFAUXCMD
\cite{satoh2008nonhydrostatic}}%DIFAUXCMD
.
}

\DIFaddend \subsection{OLAM} \label{sec:OLAMEquations}
...
\DIFadd{Within the dynamical core, FVM does not apply any explicit diffusion.  For the DCMIP test cases, the implicit diffusion of the monotonic MPDATA provides the right amount of diffusion/dissipation needed to maintain model stability and remove excess energy from the finest scales.  An absorbing layer is also available in the model for damping vertically propagating waves near the model top.
}

\DIFaddend \subsection{GEM}
...
\DIFaddbegin \subsection{\DIFadd{MPAS}}

\DIFadd{The MPAS model applies fourth-order hyperdiffusion and Smagorinsky diffusion \mbox{%DIFAUXCMD
\citep{smagorinsky1963general}}%DIFAUXCMD
, as described in \mbox{%DIFAUXCMD
\cite{skamarock2012multiscale}}%DIFAUXCMD
.  When applied to the momentum, the Laplacian is evaluated as
}\begin{align}
\DIFadd{\nabla^2 u_i = \pdiff{}{x_i} \nabla_s \cdot \vb{v} - \pdiff{\eta}{x_j},
}\end{align} \DIFadd{where $u_i$ is the edge-normal velocity defined on cell edge $i$, $\eta$ is the vertical component of the relative vorticity, computed on vertices, and $\nabla_s \cdot \vb{v}$ is the horizontal divergence on $s$ surfaces, computed on edges.  The evaluation of divergence and vorticity in this expression is described in \mbox{%DIFAUXCMD
\cite{ringler2010unified}}%DIFAUXCMD
.  The fourth-order hyperdiffusion operator is then computed by twice applying the above Laplacian operator to the momentum.
}

\DIFadd{Smagorinsky diffusion, which is often applied in atmospheric models to parameterize turbulent processes, uses a second-order Laplacian and a physically-motivated eddy viscosity $K_h$, defined in terms of Cartesian velocities $(u,v)$,
}\begin{align}
\DIFadd{K_h = c_s^2 \ell^2 \sqrt{(u_x - v_y)^2 + (u_y + v_x)^2},
}\end{align} \DIFadd{where $c_s$ is a constant parameter and $\ell$ is the grid scale.  The diffusion operator then takes the form $\nabla \cdot (K_h \nabla \psi)$ for a scalar field $\psi$.
}

%DIF > {\color{red}The evaluation of the eddy viscosity Kh is accomplished by projecting the velocities onto a tangent plane, integrating the squared terms involving the velocities in (17) over the cell, and applying Stokes theorem to transform the cell integrals to discrete line integrals around the cell edge. The evaluation is inexpensive because the prognostic normal and diagnostic tangential velocities exist, and the time-independent coefficients for the line integrals are precomputed and stored before the time integration begins.}

\subsection{\DIFadd{NICAM}}

\DIFadd{NICAM implements three types of diffusion: 3D divergence damping, fourth-order horizontal hyperdiffusion, and sixth-order vertical hyperdiffusion as described in \mbox{%DIFAUXCMD
\cite{tomita2004new}}%DIFAUXCMD
.  Specifically, the divergence damping term \mbox{%DIFAUXCMD
\citep{skamarock1992stability} }%DIFAUXCMD
aims to suppress instabilities that arise due to the time splitting scheme, and is applied to both horizontal and vertical velocities.  The hyperdiffusion operators are applied to all prognostic variables.  For tracer advection, upwinding is used to remove spurious oscillations, as described in \mbox{%DIFAUXCMD
\cite{miura2007upwind,niwa2011three}}%DIFAUXCMD
.
}

\DIFaddend \subsection{OLAM}

OLAM requires two types of artificial damping. In the upper layers of the model, vertical velocity and small-scale horizontal divergence are damped in order to attenuate gravity waves and thereby mitigate their reflection off the rigid top boundary of the domain. The damping layer is commonly applied in the uppermost 10 km of the domain, where the model top is 35 or 40 km above sea level. The damping rate is zero at the bottom of the damping layer and increases upward, usually linearly.  Throughout the model domain, vertical vorticity is filtered horizontally at the smallest resolvable scale in order to control a spurious computational mode. This vertical vorticity mode is inherent in C-staggered momentum formulations on hexagonal meshes because horizontal velocities are more numerous than twice the number of scalar (mass) values and are thus under-constrained \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{weller2012computational, weller2012controlling}}%DIFAUXCMD
}\DIFaddend . The vorticity filter is constructed so as to have zero impact on divergence at any scale.  Upwinding in the Lax-Wendroff formulation of the advection operator provides sufficient damping that no other type of filtering is required.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Temporal discretizations} \label{sec:TemporalDiscretizations}

Temporal discretizations are important for capturing the discrete dynamical evolution of the global atmosphere.  In the past two decades, a variety of new temporal discretizations have been developed, leaving behind the days when the leapfrog scheme was ubiquitous across models.  This diversity is in part because of the demands of non-hydrostatic models: unlike their hydrostatic counterparts, non-hydrostatic atmospheric models \DIFdelbegin \DIFdel{that do not use the anelastic \mbox{%DIFAUXCMD
\citep{ogura1962scale}}%DIFAUXCMD
, quasi-hydrostatic \mbox{%DIFAUXCMD
\citep{orlanski1981quasi}}%DIFAUXCMD
, pseudo-incompressible \mbox{%DIFAUXCMD
\citep{durran1989improving}}%DIFAUXCMD
, or unified approximation \mbox{%DIFAUXCMD
\citep{arakawa2009unification} }%DIFAUXCMD
}\DIFdelend must include a mechanism \DIFdelbegin \DIFdel{in the temporal discretization }\DIFdelend for dealing with vertically propagating sound waves.  These waves are meteorologically insignificant, but with a vertical grid spacing of 100 meters, a purely explicit temporal discretization \DIFaddbegin \DIFadd{of the unmodified fluid equations }\DIFaddend would require a time step size on the order of one second or less.  \DIFaddbegin \DIFadd{Consequently, sound waves are either filtered explicitly or through the use of an alternative equation set, or artificially slowed through the use of implicit temporal discretizations.  Some commonly employed alternative equation sets include the anelastic \mbox{%DIFAUXCMD
\citep{ogura1962scale}}%DIFAUXCMD
, quasi-hydrostatic \mbox{%DIFAUXCMD
\citep{orlanski1981quasi}}%DIFAUXCMD
, pseudo-incompressible \mbox{%DIFAUXCMD
...
, or unified approximation \mbox{%DIFAUXCMD
\citep{arakawa2009unification}}%DIFAUXCMD
.  These filtered equation sets generally require that a global elliptic solve be performed as prognostic variables are updated.  }\DIFaddend In this section we discuss the timestepping schemes that have been employed across the DCMIP suite of models.
...
\DIFaddbegin \DIFadd{ACME-A and Tempest both use the ARS(2,3,2) scheme described in \mbox{%DIFAUXCMD
\cite{ascher1997implicit}}%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Tempest uses the ARS(2,3,2) scheme described in \mbox{%DIFAUXCMD
\cite{ascher1997implicit}}%DIFAUXCMD
...
%DIF > \subsection{Momentum Form}
\DIFaddbegin 

%DIF > The equations of motion can similarly be formulated in terms of momentum, which we denote by
%DIF > \begin{align}
%DIF > \vb{U} = \rho \vb{u}, \qquad \vb{U}_h = \rho \vb{u}_h, \qquad \mbox{and} \qquad W = \rho w,
%DIF > \end{align} for total, horizontal, and vertical velocities, respectively.  Following (\ref{eq:DefinitionDotS}), these definitions gives rise to the generalized vertical momentum,
%DIF > \begin{align}
%DIF > \dot{S} = W - \vb{U}_h \cdot \nabla_s z
%DIF > \end{align}  Then multiplying (\ref{eq:PrognosticUhEquationK2Sb}) through by $\rho$ and using (\ref{eq:ContinuityEquation}) gives
%DIF > \begin{align}
%DIF > \pdiff{\vb{U}_h}{t} =& - W (\vb{u}_h \cdot \nabla \vb{k}) - \rho \nabla_s K_2 - \zeta_s \vb{k} \times \vb{U}_h - \pdiff{}{s} \left( \dot{S} \vb{u}_h \right) - \rho \nabla_z \Phi - \nabla_z p - [2 \vg{\Omega} \times \rho \vb{u}]_z
%DIF > \end{align}
%DIF > \begin{align}
...
%DIF > \begin{align}
%DIF > \pdiff{W}{t} =&\left( \pdiff{s}{z} \right) \left[ - \rho \pdiff{\Phi}{s} - \pdiff{p}{s} \right] - (\nabla \cdot \vb{U} w) - (2 \vg{\Omega} \times \vb{U}_h) \cdot \vb{k}
%DIF > \end{align}

\DIFaddend \subsection{Covariant Component Formulation}
